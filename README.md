# Data Science Portfolio
## Alam Arana Wilson

Data Analyst en formación con base en Data Science, con experiencia en ingeniería y optimización industrial. Enfocado en análisis estructurado de datos utilizando SQL y Python para generar insights accionables.

---

## 📌 Sobre mí

Ingeniero con experiencia en mejora continua, Lean Manufacturing y optimización de procesos industriales. Actualmente cursando un Diplomado en Data Science, desarrollando proyectos enfocados en limpieza de datos, análisis exploratorio (EDA) y visualización para toma de decisiones basada en datos.

---

## 🛠 Habilidades Técnicas

- SQL
- Python
- Exploratory Data Analysis (EDA)
- Data Cleaning & Transformation
- Data Visualization
- Lean Manufacturing
- DMAIC & Root Cause Analysis

---

## 📂 Proyectos

### 🛒 [Instacart – Exploratory Data Analysis](https://github.com/alam-arana/instacart-eda)
Análisis de hábitos de compra, limpieza y preprocesamiento de datos de pedidos, productos y clientes. Identificación de patrones de consumo y frecuencia de reorden.

### 🎬 Netflix Dataset – Exploratory Data Analysis
Análisis de tendencias por género, ratings y duración de contenido. Limpieza y transformación de datos para detección de patrones relevantes.

---

## 📬 Contacto

LinkedIn: https://www.linkedin.com/in/alam-arana-wilson-79815a324/
GitHub: https://github.com/Mictlam
